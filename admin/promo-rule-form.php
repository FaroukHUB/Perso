<?php
/**
 * PERSONNALY Admin - Formulaire Règle Promotionnelle
 * Création et édition de règles promo
 */
...
$rule = $id ? $promoModel->findById($id) : null;
$isEdit = $rule !== null;
    ...
    if (empty($errors)) {
        try {
            if ($isEdit) {
                ...
                $id = $promoModel->create($data);
            }
            redirect('/admin/promo-rules.php?success=1');
        } catch (Exception $e) {
...
<head>
    ...
    <title><?= $isEdit ? 'Modifier' : 'Nouvelle' ?> Règle Promo - PERSONNALY Admin</title>
        ...
        <main class="main-content">
            <div class="page-header">
                <div style="display: flex; align-items: center; gap: 16px;">
                    <a href="/admin/promo-rules.php" class="btn-back">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <path d="M19 12H5M12 19l-7-7 7-7"/>
                        </svg>
                    </a>
                    <h1 class="page-title"><?= $isEdit ? 'Modifier la' : 'Nouvelle' ?> <span>Règle Promo</span></h1>
                </div>
            </div>
                ...
                <div class="form-grid">
                        ...
                        <div class="form-card">
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="description" name="description" rows="2"
                                          placeholder="Notes internes..."><?= h($rule['description'] ?? '') ?></textarea>
                            </div>
                        </div>
                            ...
                            <div class="form-row">
                                <div class="form-group" style="flex: 1;">
                                    ...
                                    <select id="condition_type" name="condition_type" onchange="updateConditionUI()">
                                        <?php foreach (PromoRule::CONDITION_TYPES as $key => $label): ?>
                                            <option value="<?= $key ?>" <?= ($rule['condition_type'] ?? 'panier_min') === $key ? 'selected' : '' ?>>
                                                <?= h($label) ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                        ...
                        <div class="form-card">
                            ...
                            <div class="form-group">
                                ...
                                <select id="offer_type" name="offer_type" onchange="updateOfferUI()">
                                    <?php foreach (PromoRule::OFFER_TYPES as $key => $label): ?>
                                        <option value="<?= $key ?>" <?= ($rule['offer_type'] ?? 'reduction') === $key ? 'selected' : '' ?>>
                                            <?= h($label) ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div class="form-group" id="offer-value-group">
                                ...
                                <select id="offer_value" name="offer_value">
                                    ...
                                    <?php foreach ($products as $prod): ?>
                                        <option value="<?= $prod['id'] ?>" <?= ($rule['offer_value'] ?? '') == $prod['id'] ? 'selected' : '' ?>>
                                            <?= h($prod['name']) ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div class="form-row" id="discount-group">
                                <div class="form-group" style="flex: 1;">
                                    ...
                                    <select id="discount_type" name="discount_type">
                                        <?php foreach (PromoRule::DISCOUNT_TYPES as $key => $label): ?>
                                            <option value="<?= $key ?>" <?= ($rule['discount_type'] ?? 'aucun') === $key ? 'selected' : '' ?>>
                                                <?= h($label) ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                        ...
                        <div class="form-card">
                            ...
                            <div class="form-group">
                                ...
                                <select id="display_position" name="display_position">
                                    <?php foreach (PromoRule::DISPLAY_POSITIONS as $key => $label): ?>
                                        <option value="<?= $key ?>" <?= ($rule['display_position'] ?? 'both') === $key ? 'selected' : '' ?>>
                                            <?= h($label) ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                    ...
                    <div class="form-sidebar">
                        ...
                        <div class="form-card">
                            ...
                            <?php if ($isEdit): ?>
                                <div class="stat-box">
                                    <span class="stat-value"><?= (int) ($rule['current_uses'] ?? 0) ?></span>
